...
// ----------- Ajusta renderOrderDetails para agregar botones de llamada y WhatsApp cliente ----------
...
window.takeOrder = function(orderId) {
  ...
  }, "Pedido asignado, ¡empieza tu servicio!", true);
};
...
window.releaseOrder = function(orderId) {
  updateOrder(orderId, {
    ...
    driver_name: "",
    driver_contact: ""
  }, "Pedido liberado, ahora está disponible para otro repartidor.", true);
};
...
window.nextStatus = function(orderId) {
  ...
  if (idx < STATUS_LIST.length-1) {
    ...
    updateOrder(orderId, {estado: next}, "Estado actualizado", false, true);
  }
};

window.changeStatusManual = function(orderId) {
  let st = document.getElementById('manualStatus').value;
  updateOrder(orderId, {estado: st}, "Estado actualizado", false, true);
};

/**
 * updateOrder ahora permite actualizar y luego recargar la pantalla de detalles o ir a listados según flags.
 * @param {string} orderId 
 * @param {Object} changes 
 * @param {string} msg 
 * @param {boolean} reloadDetails - si true, recarga detalles del pedido, si false, se queda igual, si undefined, va a listado
 * @param {boolean} stayOnDetails - si true, no sale de detalles
 */
function updateOrder(orderId, changes, msg="Estatus actualizado", reloadDetails=false, stayOnDetails=false) {
  let data = {
    id_pedido: orderId,
    ...changes
  };
  fetch(SHEETDB_URL + "/id_pedido/" + orderId, {
   ...
   .then(()=>{
     ...
     // Si es liberar pedido o cambio de estatus, recarga detalles en vez de salir
     if (reloadDetails || stayOnDetails) {
       setTimeout(()=>{
         // refresca datos individuales del pedido y vuelve a renderizar detalles
         fetch(ORDER_SHEET_URL)
          .then(r=>r.text())
          .then(csv=>{
            orders = parseCSV(csv);
